...
if (!class_exists('MappIntelligenceDatalayer')) {
    class MappIntelligenceDatalayer
    {
        private $datalayer = array(); // will be exposed to client as window._ti
        private $current_product = array(); // temporary savepoint for product data during product archive loop
         ...
         */
        private function product_to_datalayer($datalayer_key, $data, $write_to_array, $is_collection_child = false)
        {
            if ($write_to_array) {
                $this->current_product[$datalayer_key] = $data;
            } elseif ($is_collection_child) {
				$this->current_product[$datalayer_key] = $data;
				$this->datalayer[$datalayer_key] = $this->datalayer[$datalayer_key] . ';' . $data;
			}
            else {
        ...
        protected function get_page_info()
        {
            ...
            if (is_singular()) {
                ...
                $this->datalayer["taxonomies"] = $this->get_taxonomies($post_id);
                ...
                $this->datalayer['pageNumber'] = $pageNumber > 0 ? strval($pageNumber) : '1';
                $orderBy = get_query_var('orderby');
                $this->datalayer['orderBy'] = $orderBy !== "" ? $orderBy : "default";
                // loop through the different types of archives
                $f = array('category', 'tag', 'tax', 'author', 'year', 'month', 'day', 'time', 'date');
                for ($i=0; $i<count($f); $i++) {
                    if (call_user_func('is_' . $f[$i])) { // is_category(), is_tag() etc.
						$this->datalayer['contentCategory'] = 'archive';
                        $this->datalayer['contentSubcategory'] =  $f[$i] . '-' . $post_type;
                        if ($f[$i] === 'category') {
                            $_post_cats = get_the_category();
                            $this->datalayer['category'] = array();
            ...
            if (is_front_page()) {
				...
				if(is_shop()) {
				$this->datalayer['contentCategory'] = isset($this->datalayer['contentCategory']) ?
					$this->datalayer['contentCategory'] : 'shop-startpage';
				}
         ...
         */
        protected function get_search_info()
        {
            if (is_search()) {
                global $wp_query;
         ...
         */
        protected function get_404_info()
        {
            if (is_404()) {
         ...
         */
        protected function get_order_info()
        {
            global $wp;
            if (defined('WC_VERSION') && is_order_received_page()) {
                ...
                foreach ($products as $product) {
                    $product_id = $product->get_variation_id() === 0 ?
                        $product->get_product_id() : $product->get_variation_id();
                    $this->get_product_info(true, $product_id);
                    $this->datalayer['products'][$_datalayer_product_index]['productQuantity'] =
                        $product->get_quantity();
                    $this->datalayer['products'][$_datalayer_product_index]['productCost'] = $product->get_subtotal();
                ...
                $this->datalayer['paymentMethod'] = $order->get_payment_method_title();
         ...
         */
        protected function mappify_order_items()
        {
            if (defined('WC_VERSION') && is_order_received_page()) {
                ?>
                <script>
         ...
         */
        protected function get_product_info($write_to_array = false, $product_id = 0, $is_collection_child = false)
        {
            if (defined('WC_VERSION') && is_shop() ||
                is_product_category() || is_product_tag() ||
                is_product() || is_order_received_page()) {
                // handling beginning of loop loop in case of product archive
                if ($write_to_array) {
                    $this->current_product = array();
                }
                ...
                if ($product!== false) {
                    ...
                    $this->product_to_datalayer('productName', $product->get_title(), $write_to_array, $is_collection_child);
                    $this->product_to_datalayer('productCost', $product->get_price(), $write_to_array, $is_collection_child);
                    $this->product_to_datalayer('productId', strval($product_id), $write_to_array, $is_collection_child);
                    $this->product_to_datalayer('productSKU', $product->get_sku(), $write_to_array, $is_collection_child);
                    $this->datalayer['currency'] = get_woocommerce_currency();
                    $this->product_to_datalayer('productQuantity', '1', $write_to_array, $is_collection_child);

                    // stock
                    $stock = $product->get_stock_status();
                    if ($stock === 'outofstock') {
                        $this->product_to_datalayer('productSoldOut', '1', $write_to_array, $is_collection_child);
                    }
					...
					if($product->get_type() === 'variation') {
						...
						if(count($attributes) > 0) {
							$this->product_to_datalayer('productAttributes', $attributes, $write_to_array, $is_collection_child);
						}
					}
                }
                ...
                if ($write_to_array || $is_collection_child) {
                    $this->datalayer['products'] = isset($this->datalayer['products']) ?
                        $this->datalayer['products'] : array();
                    $this->current_product['taxonomies'] = $this->get_taxonomies($product_id);
                    array_push($this->datalayer['products'], $this->current_product);
                    $this->current_product = array();
                }

                // handling Product collection
				if($is_collection_child === false &&
		...
		private function add_product_categories_based_on_taxonomies_helper(&$object) {
        	if(isset($object['taxonomies']['product_cat'])) {
				...
				if(isset($object['taxonomies']['product_cat'][0])) {
					$object['productCategory'] = $object['taxonomies']['product_cat'][0];
				}
				if(isset($object['taxonomies']['product_cat'][1])) {
					$object['productSubcategory'] = $object['taxonomies']['product_cat'][1];
				}
			}
		}
        ...
        private function get_taxonomies($post_id)
		{
			...
			$child = 0;
			if($post_type === 'product_variation') { // variations don't have taxonomies, but their parents have
				$child = $post_id;
		...
		public function add_product_categories_based_on_taxonomies() {
			if(isset($this->datalayer['taxonomies']['product_cat'])) {
				$this->add_product_categories_based_on_taxonomies_helper($this->datalayer);
			}
			if(isset($this->datalayer['products'])) {
				foreach ($this->datalayer['products'] as &$product ) {
					$this->add_product_categories_based_on_taxonomies_helper($product);
				}
			}
		}
		...
		private function handle_multiple_product_taxonomies() {
			...
			foreach($this->datalayer['products'] as $product) { // get all taxonomy keys from all products
				$all_taxonomies = array_merge($all_taxonomies, array_keys($product['taxonomies']));
			}
			$all_taxonomies = array_unique($all_taxonomies);
			// get the max. count of each taxonomy in all products
			$max_tax_value_count = array();
			foreach ($all_taxonomies as $taxonomy) {
				//init based on group product
				$max_tax_value_count[$taxonomy] = isset($this->datalayer['taxonomies'][$taxonomy]) ?
					count($this->datalayer['taxonomies'][$taxonomy]) : 0;
			}
			foreach($this->datalayer['products'] as $product) {
			...
			for($i = 0; $i < $product_count; $i++) {
				...
				foreach ($max_tax_value_count as $taxonomy_name => $taxonomy_values) {
					for($j = 0; $j < $max_tax_value_count[$taxonomy_name]; $j++) {
						$additional_value = isset($product['taxonomies'][$taxonomy_name][$j]) ?
							$product['taxonomies'][$taxonomy_name][$j] : '';
						$current_value = isset($this->datalayer['taxonomies'][$taxonomy_name][$j]) ?
         ...
         */
        protected function get_product_status($status)
        {
		 ...
		 */
        protected function create_is_collection_info()
		{
			$output = '';
			foreach (explode(';', $this->datalayer['taxonomies']['product_type'][0]) as $item) {
				$output .= ($item === 'grouped' ? '1' : '0') . ';';
			}
			$this->datalayer['productCollection'] = preg_replace('/;$/', '', $output);
		}
		 ...
		 * @return false|array
		 */
        protected function create_datalayer($config)
        {
            $filter_keys = isset($config['General']['filterKeys']) ?
                explode(',', $config['General']['filterKeys']) : array();
		 ...
		 * @param $is_ti boolean
		 */
        protected function track_add_product_single($is_ti)
        {
            if (defined('WC_VERSION') && is_product()) {
                ?>
                <script>
		 ...
		 * @param $is_ti boolean
		 */
        protected function track_add_product_archive($is_ti)
        {
            if (defined('WC_VERSION') && (is_shop() || is_product_category() || is_product_tag() || is_product())) {
                ?>
                <script>
		 ...
		 * @param $is_ti boolean
         */
        protected function product_variant_events($is_ti)
        {
            if (defined('WC_VERSION') && is_product()) {
                ?>
                <script>
        ...
        public function create_gtm_product_array()
		{
			if (defined('WC_VERSION') && (is_shop() ||
				...
				if(is_order_received_page() && isset($this->datalayer['products'])) {
					$this->datalayer[$gtm_array_key] = array();
					for ($i = 0; $i < count($this->datalayer['products']); $i++)
					{
						array_push($this->datalayer[$gtm_array_key], $this->gtm_product_parser($this->datalayer['products'][$i]));
					}
				} else {
					$this->datalayer[$gtm_array_key] = array($this->gtm_product_parser($this->datalayer));
				}
			}
		}
		...
		{
			$gtm_product_array = array();
			$keys = array('id', 'cost', 'quantity', 'name', 'collection');

			foreach ($keys as $key) {
				$this->create_gtm_product_entry($gtm_product_array, $product_entry, $key);
			}
			if (isset($product_entry['productSoldOut'])) {
				$gtm_product_array['soldOut'] = $product_entry['productSoldOut'] === '1';
			}
			...
			if (isset($product_entry['shoppingCartStatus'])) {
				...
				$gtm_product_array['status'] = $this->datalayer['shoppingCartStatus'];
			}
			...
			if(isset($product_entry['productId'])  && isset($product_entry['productName'])) {
				if(preg_match('/;/', $product_entry['productId'])) { // product collections
					preg_match_all( "/([0-9]+?)(?:;|$)/" , $product_entry['productId'], $ids);
					$ids = $ids[1];
					preg_match_all( "/(.+?)(?:;|$)/" , $product_entry['productName'], $names);
					$names = $names[1];
					$gtm_product_array['id'] = '';
					for ($i = 0; $i < count($ids); $i++) {
						$gtm_product_array['id'] = $gtm_product_array['id'] . ';' . $ids[$i] . ' - ' . $names[$i];
					}
					$gtm_product_array['id'] = substr($gtm_product_array['id'], 1);
				} else {
					$gtm_product_array['id'] = $product_entry['productId'] . ' - ' . $product_entry['productName'];
				}
...
//					$reversed_tax_values = array_reverse($tax_values);
			...
			if(isset($product_entry['productAttributes'])) {
				foreach($product_entry['productAttributes'] as $attribute_name => $attribute_value)
				{
					$gtm_product_array[ 'attribute_' . $attribute_name] = $attribute_value;
				}
			}
		...
		private function create_gtm_product_entry(&$product_array, &$product_entry, $key )
		{
